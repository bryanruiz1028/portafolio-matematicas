# Ecuación de la elipse — Índice del curso

tags: #geometría analítica #notebooklm #youtube
Generado con NotebookLM: 2026-04-25
Cuaderno NotebookLM ID: `cdf52aad-9879-4e94-97b4-92dc78f02ae0`

> [!info] 🤖 Curso generado con NotebookLM
> Este curso fue generado automáticamente desde 21 videos de YouTube usando NotebookLM.
> Cada clase, presentación y guía fue generada por IA basándose en el contenido de los videos.

---

## Estructura por clases

| Clase | Tema | Estado |
|-------|------|--------|
| [[Clase 01 - Trazado y elementos de la elipse Ecuación canónica y ordina|01]] | Ellipse tracing and elements | Introduction + Canonical equa | ⏳ |
| [[Clase 02 - Gráfica y elementos de la elipse de la ecuación a la gráfic|02]] | Graph and elements of the ellipse knowing the standard equat | ⏳ |
| [[Clase 03 - Ecuación ordinaria de la elipse Gráfica, Vértices y Focos|03]] | Standard equation of the ellipse given the graph | Example 3 | ⏳ |
| [[Clase 04 - La Elipse Conversión entre Ecuaciones Canónica y General|04]] | Ellipse | Converting from the canonical equation to the gene | ⏳ |
| [[Clase 05 - Conversión de la ecuación general de la elipse a su forma ca|05]] | Ellipse | Converting from the general equation to the canoni | ⏳ |
| [[Clase 06 - Gráfica y elementos de la elipse a partir de su ecuación gen|06]] | Graph and elements of the ellipse, knowing its general equat | ⏳ |

---

## 📂 Materiales por clase

| Clase | 📊 Presentación | 📖 Guía de estudio |
|-------|----------------|-------------------|
| Clase 01 | — | [[📖 Guía de estudio - Clase 01 — 📖 Guía de estudio — Clase 01 Trazo y elementos de]] |
| Clase 02 | — | [[📖 Guía de estudio - Clase 02 — 📖 Guía de estudio — Clase 02 Gráfica y elementos]] |
| Clase 03 | — | [[📖 Guía de estudio - Clase 03 — 📖 Guía de estudio — Clase 03 Ecuación estándar de]] |
| Clase 04 | — | [[📖 Guía de estudio - Clase 04 — 📖 Guía de estudio — Clase 04 Ecuaciones de la Eli]] |
| Clase 05 | — | [[📖 Guía de estudio - Clase 05 — 📖 Guía de estudio — Clase 05 De la Ecuación Gener]] |
| Clase 06 | — | [[📖 Guía de estudio - Clase 06 — 📖 Guía de estudio — Clase 06 Gráfica y elementos]] |

---

## 📝 Recursos por bloque

| Bloque | Clases | Evaluación | Planificación |
|--------|--------|-----------|--------------|
| Bloque 1 | Clases 1, 2, 3 | [[📝 Evaluación - Bloque 1 — Ellipse tracing and elements y Canonical equation|📝 Evaluación]] | [[📋 Planificación - Bloque 1 — Ellipse tracing and elements y Canonical equation|📋 Planificación]] |
| Bloque 2 | Clases 4, 5, 6 | [[📝 Evaluación - Bloque 2 — Gráfica y elementos de la Elipse conociendo la ecu|📝 Evaluación]] | [[📋 Planificación - Bloque 2 — Gráfica y elementos de la Elipse conociendo la ecu|📋 Planificación]] |

---

## 📈 Progreso

| Métrica | Valor |
|---------|-------|
| Clases generadas | 6 / 6 |
| Presentaciones | 0 |
| Guías de estudio | 6 |
| Evaluaciones | 2 |
| Planificaciones | 2 |

---

## 🔗 Conexiones

- [[Cursos/00 - Mapa de cursos|Mapa de cursos]]
- [[DASHBOARD|Dashboard general]]
- [[🤖 Guía - NotebookLM y presentaciones|Guía NotebookLM]]
