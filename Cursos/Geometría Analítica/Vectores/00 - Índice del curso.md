# Vectores — Índice del curso

tags: #geometría analítica #notebooklm #youtube
Generado con NotebookLM: 2026-04-25
Cuaderno NotebookLM ID: `9be2b3b9-1fbb-4317-9d25-8a8706eb0510`

> [!info] 🤖 Curso generado con NotebookLM
> Este curso fue generado automáticamente desde 22 videos de YouTube usando NotebookLM.
> Cada clase, presentación y guía fue generada por IA basándose en el contenido de los videos.

---

## Estructura por clases

| Clase | Tema | Estado |
|-------|------|--------|
| [[Clase 01 - Introducción a los Vectores, Magnitud y Componentes|01]] | Vectors Introduction | What is a vector and its characterist | ⏳ |
| [[Clase 02 - Representación gráfica de vectores por componentes, suma y r|02]] | Graphical representation of vectors by components + Suma y r | ⏳ |
| [[Clase 03 - Suma de vectores Método gráfico|03]] | Vector Addition Using the Graphical Method | Example 1 + Vec | ⏳ |
| [[Clase 04 - Componentes Rectangulares y Ángulo de un Vector|04]] | Componentes Rectangulares de un vector | Ejemplo 1 + Compone | ⏳ |
| [[Clase 05 - Suma de vectores por componentes rectangulares|05]] | Vector Sum by Rectangular Components | Example 1 + Vector Su | ⏳ |
| [[Clase 06 - Multiplicación por Escalar y Ángulo entre Vectores|06]] | Multiplying a Vector by a Scalar | Example 1 + Multiplying a | ⏳ |

---

## 📂 Materiales por clase

| Clase | 📊 Presentación | 📖 Guía de estudio |
|-------|----------------|-------------------|
| Clase 01 | — | [[📖 Guía de estudio - Clase 01 — Introducción a los Vectores, Magnitud y Componentes]] |
| Clase 02 | — | [[📖 Guía de estudio - Clase 02 — Representación gráfica de vectores por componentes, suma y r]] |
| Clase 03 | — | [[📖 Guía de estudio - Clase 03 — Suma de vectores Método gráfico]] |
| Clase 04 | — | [[📖 Guía de estudio - Clase 04 — Componentes Rectangulares y Ángulo de un Vector]] |
| Clase 05 | — | [[📖 Guía de estudio - Clase 05 — Suma de vectores por componentes rectangulares]] |
| Clase 06 | — | [[📖 Guía de estudio - Clase 06 — Multiplicación por Escalar y Ángulo entre Vectores]] |

---

## 📝 Recursos por bloque

| Bloque | Clases | Evaluación | Planificación |
|--------|--------|-----------|--------------|
| Bloque 1 | Clases 1, 2, 3 | [[Evaluación - Bloque 1 Introducción a los Vectores Magnitud y Componentes|📝 Evaluación]] | [[📋 Planificación - Bloque 1 — Introducción a los Vectores, Magnitud y Componentes|📋 Planificación]] |
| Bloque 2 | Clases 4, 5, 6 | [[Evaluación - Bloque 2 Multiplicación por Escalar y Ángulo entre Vectores|📝 Evaluación]] | [[📋 Planificación - Bloque 2 — Multiplicación por Escalar y Ángulo entre Vectores|📋 Planificación]] |

---

## 📈 Progreso

| Métrica | Valor |
|---------|-------|
| Clases generadas | 6 / 6 |
| Presentaciones | 3 |
| Guías de estudio | 6 |
| Evaluaciones | 5 |
| Planificaciones | 2 |

---

## 🔗 Conexiones

- [[Cursos/00 - Mapa de cursos|Mapa de cursos]]
- [[DASHBOARD|Dashboard general]]
- [[🤖 Guía - NotebookLM y presentaciones|Guía NotebookLM]]
